...
xdescribe('TeacherClassView', () => // describe 'when logged out', ->
...
describe('when logged in', function() {
  ...
  describe('when python classroom', function() {
    ...
    describe('the Students tab', () => beforeEach(function(done) {
    ...
    return xdescribe('Export Student Progress (CSV) button', () => it('downloads a CSV file', function(done) {
      spyOn(window, 'saveAs').and.callFake((blob, fileName) => {
        ...
        reader.onload = event => {
          ...
          for (var line of Array.from(lines)) {
            ...
            expect(simplerLine.match(/[^,]+/g).length).toBe(5 + (this.releasedCourses.length * 2) + 1);
    ...
    return describe('when the student has a starter license', () => describe('and the course is NOT covered by starter licenses', function() {
      beforeEach(function(done) {
        ...
        this.starterStudent = this.students.find(function(s) {
          ...
          return s.get('products').length && (s.get('products')[0].prepaid === starterId);
        });
