  ...
  CocoRouter = class CocoRouter extends Backbone.Router {
    static initClass() {
      ...
      this.prototype.routes = {
        ''() {
          ...
          if (!me.isAnonymous() && !me.isStudent() && !me.isTeacher() && !me.isAdmin() && !me.hasSubscription() && !me.isAPIClient() && !paymentUtils.hasTemporaryPremiumAccess()) {
            delete window.alreadyLoadedView;
          ...
          if (me.isAPIClient()) {
            ...
            return this.navigate("/api-dashboard", {trigger: true, replace: true});
          }
        ...
        'teachers/resources': me.useChinaHomeView() ? go('teachers/ResourceHubView', { redirectStudents: true }) : go('core/SingletonAppVueComponentView'),
    ...
    redirectHome() {
      ...
      const homeUrl = (() => { switch (false) {
        ...
        case !me.isAPIClient(): return "/api-dashboard";
