...
module.exports = (LevelDialogueView = (function() {
  LevelDialogueView = class LevelDialogueView extends CocoView {
    ...
    onSpriteDialogue(e) {
      ...
      if (e.mood === 'debrief') {
        if (e.sprite.thangType.get('poseImage') != null) {
          return this.$el.find('.dialogue-area').append($('<img/>').addClass('embiggen').attr('src', '/file/' + e.sprite.thangType.get('poseImage')));
        } else {
          return this.$el.find('.dialogue-area').append($('<img/>').attr('src', e.sprite.thangType.getPortraitURL()));
        }
      }
    }
