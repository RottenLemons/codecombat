...
const {downTheChain} = require('lib/world/world_utils');
...
module.exports = (DocFormatter = class DocFormatter {
  ...
  fillOutDoc() {
    ...
    if (this.options.isSnippet) {
      ...
      if (this.doc.type === 'function') {
        ...
        this.doc.shortName = (() => { switch (this.options.language) {
          case 'coffeescript': return `${ownerName}${ownerName === '@' ? '' : '.'}${this.doc.name}`;
          case 'python': return `${ownerName}.${this.doc.name}`;
          case 'lua': return `${ownerName}.${this.doc.name}`;
          default: return `${ownerName}.${this.doc.name};`;
        } })();
      }
      ...
      if (['javascript', 'java', 'cpp'].includes(this.options.language)) {
    ...
    const toTranslate = [{obj: this.doc, prop: 'description'}, {obj: this.doc, prop: 'example'}];
    ...
    for (var {obj, prop} of Array.from(toTranslate)) {
      var originalVal, val;
      if (!(obj[prop] != null ? obj[prop][this.options.language] : undefined) && (obj[prop] != null ? obj[prop].javascript : undefined)) {
        // Translate into chosen code language.
        if (prop === 'example') {
          // Try to autogenerate the language-specific code example
          obj[prop][this.options.language] = aetherUtils.translateJS(obj[prop].javascript, this.options.language, false);
        } else {
          if (['lua', 'coffeescript', 'python'].includes(this.options.language)) {
            // These are mostly the same, so use the Python or JavaScript ones if language-specific ones aren't available
            obj[prop][this.options.language] = obj[prop].python != null ? obj[prop].python : obj[prop].javascript;
          } else if (['java', 'cpp'].includes(this.options.language)) {
            // These are mostly the same, so use the JavaScript ones if language-specific ones aren't available
            obj[prop][this.options.language] = obj[prop].javascript;
          }
        }
      }
    ...
    if (this.options.useHero) {
      ...
      if (thisToken[this.options.language]) {
        ...
        if (this.doc.args) {
          return (() => {
            const result = [];
            for (arg of Array.from(this.doc.args)) {               if (arg.example) {
                result.push(arg.example = arg.example.replace(thisToken[this.options.language], 'hero'));
              }
            }
            return result;
          })();
  ...
  formatPopover() {
    ...
    let content = popoverTemplate({
      ...
      value: this.formatValue(undefined, true),
      marked,
      argumentExamples,
    ...
    content = content.replace(/\#\{(.*?)\}/g, (s, properties) => this.formatValue(downTheChain(owner, properties.split('.'))));
    return content = content.replace(/{([a-z]+)}([^]*?){\/\1}/g, (s, language, text) => {
      if (language === this.options.language) { return text; }
      if ((language === 'javascript') && ['java', 'cpp'].includes(this.options.language)) { return text; }
      return '';
    });
  }
  ...
  formatValue(v, isTopLevel) {
    ...
    if ((v == null) && (this.doc.type === 'function') && isTopLevel) { return '[Function]'; }
    if (v == null) {
