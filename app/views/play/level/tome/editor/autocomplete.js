...
module.exports = (Autocomplete = (function() {
  ...
  Autocomplete = class Autocomplete {
    ...
    addCodeCombatSnippets(level, spellView, e) {
      let attackEntry, content, entry, left, name, replacement;
      ...
      const usedAutocompleteReplacement = [];
      for (var group in e.propGroups) {
        ...
        for (var prop of Array.from(props)) {
          ...
          var doc = _.find(((left1 = e.allDocs['__' + prop]) != null ? left1 : []), function(doc) {
            if (doc.owner === owner) { return true; }
            return ((owner === 'this') || (owner === 'more')) && ((doc.owner == null) || (doc.owner === 'this'));
          });
          if (['java', 'cpp'].includes(e.language) && !__guard__(doc != null ? doc.snippets : undefined, x => x[e.language]) && __guard__(doc != null ? doc.snippets : undefined, x1 => x1.javascript)) {
          ...
          if (['lua', 'coffeescript', 'python'].includes(e.language) && !__guard__(doc != null ? doc.snippets : undefined, x2 => x2[e.language]) && (__guard__(doc != null ? doc.snippets : undefined, x3 => x3.python) || __guard__(doc != null ? doc.snippets : undefined, x4 => x4.javascript))) {
            // These are mostly the same, so use the Python or JavaScript ones if language-specific ones aren't available
            doc.snippets[e.language] = __guard__(doc != null ? doc.snippets : undefined, x5 => x5.python) || doc.snippets.javascript;
          }
          if (__guard__(doc != null ? doc.snippets : undefined, x6 => x6[e.language])) {
            ({
              name
            } = doc);
            replacement = _.find(autocompleteReplacement, el => el.name === name);
            if (replacement) {
              usedAutocompleteReplacement.push(replacement.name);
            }
            content = __guard__(__guard__(replacement != null ? replacement.snippets : undefined, x8 => x8[e.language]), x7 => x7.code) || doc.snippets[e.language].code;
      ...
      for (replacement of Array.from(autocompleteReplacement)) {
        if (Array.from(usedAutocompleteReplacement).includes(replacement.name)) { continue; }
        if (!replacement.snippets) { continue; }
        entry = {
          content: __guard__(__guard__(replacement != null ? replacement.snippets : undefined, x10 => x10[e.language]), x9 => x9.code),
          meta: $.i18n.t('keyboard_shortcuts.press_enter', {defaultValue: 'press enter'}),
          name: replacement.name,
          tabTrigger: __guard__(replacement.snippets != null ? replacement.snippets[e.language] : undefined, x11 => x11.tab),
          importance: replacement.autoCompletePriority != null ? replacement.autoCompletePriority : 1.0
        };
        snippetEntries.push(entry);
      }
