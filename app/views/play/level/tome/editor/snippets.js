    ...
    if (cursor.column > 0) {
      ...
      if (prevWord.length > 0) {
        ...
        if (prevWordIndex === 0) {
          ...
          for (var completer of Array.from(editor.completers)) {
            if (completer.completions != null) {
              for (var completion of Array.from(completer.completions)) {
  ...
  return {
    getCompletions(editor, session, pos, prefix, callback) {
      ...
      SnippetManager.getActiveScopes(editor).forEach(function(scope) {
        ...
        return (() => {
          const result = [];
          for (var s of Array.from(snippets)) {
            var left;
            var caption  = s.name || s.tabTrigger;
            if (!caption) { continue; }
            if (/^['"]/.test(caption) && emptyBeginning) { continue; } // don't show string completions at the end of line
            var [snippet, fuzzScore] = Array.from(scrubSnippet(s.content, caption, line, prefix, pos, lang, autoLineEndings, s.captureReturn));
