...
${aetherUtils.translateJS(a.slice(13, +(a.length-4) + 1 || undefined), this.language, false)}
\`\`\``; }
    ...
    // Intro types don't have solutions yet, so don't show them for now:
      ...
      for (level of Array.from((this.levels != null ? this.levels.models : undefined))) {
        ...
        if (programmableMethod) {
          ...
          try {
            if (utils.isCodeCombat) {
              defaultCode = programmableMethod.languages[solutionLanguage] || ((this.language === 'cpp') && aetherUtils.translateJS(programmableMethod.source, 'cpp')) || programmableMethod.source;
            } else {
              defaultCode = programmableMethod.languages[level.get('primerLanguage') || this.language] || ((this.language === 'cpp') && aetherUtils.translateJS(programmableMethod.source, 'cpp')) || programmableMethod.source;
            }
