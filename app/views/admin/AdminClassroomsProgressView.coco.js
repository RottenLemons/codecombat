...
module.exports = (AdminClassroomsProgressView = (function() {
  AdminClassroomsProgressView = class AdminClassroomsProgressView extends RootView {
    ...
    buildProgressData() {
      ...
      .then(results => {
        let campaigns, classroom, courses, prepaids, teacher, teachers;
        let course, c;
        ...
        const excludedCourseIds = ((() => {
          const result = [];
          for (course of Array.from(courses.filter(c => c.free || (c.releasePhase !== 'released')))) {             result.push(course._id);
          }
          return result;
        })());
        ...
        .then(levelSessions => {
          ...
          for (var classroomId in classroomLicenseCourseLevelMap) { //when classroomId is '573ac4b48edc9c1f009cd6be'
