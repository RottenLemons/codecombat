...
module.exports = (ScriptManager = (ScriptManager = (function() {
  ScriptManager = class ScriptManager extends CocoClass {
    ...
    constructor(options) {
      ...
      super(options);
      this.originalScripts = this.filterScripts(options.scripts);
    ...
    setScripts(newScripts) {
      this.originalScripts = this.filterScripts(newScripts);
      this.quiet = true;
      this.initProperties();
    ...
    filterScripts(scripts) {
      return _.filter(scripts, function(script) {
        ...
        if (!['Intro Dialogue', 'Failure Dialogue',  'Success Dialogue'].includes(script.id)) { return true; }
        if (serverConfig.enableNarrative == null) { return false; }
        if ((me.get('testGroupNumber') % 8) < 4) { return false; } // Groups 0-3 dont see narrative
        return true;
      });
    }
